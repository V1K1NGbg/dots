-- Hyprland 0.55+ profile for the Framework Laptop 16.
...
local function configure_monitors()
    ...
    if external and current_panel then
        hl.monitor({ output = external, mode = "2560x1440@165", position = "0x80", scale = 1 })
        hl.monitor({ output = current_panel, mode = "2560x1600@165", position = "2560x0", scale = 1 })
...
-- Preserve the familiar geometry with native Hyprland motion.
...
-- Persistent workspaces and application routing.
...
for _, class in ipairs({
    "Blueman-manager", "pinentry.*", "org.pulseaudio.pavucontrol",
...
bind(mod .. " + S", hl.dsp.exec_cmd("nixfwbtw-hypr-keybinds"))
bind(mod .. " + L", hl.dsp.exec_cmd("hyprlock"))
bind(mod .. " + P", hl.dsp.exec_cmd("nixfwbtw-hypr-screenshot"))
...
-- Cycle the current workspace through four native layouts.
